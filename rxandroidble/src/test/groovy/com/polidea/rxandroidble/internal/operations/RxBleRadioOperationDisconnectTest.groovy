...
public class RxBleRadioOperationDisconnectTest extends Specification {
    ...
    def setup() {
        mockGattCallback.getBluetoothGatt() >> Observable.just(mockBluetoothGatt)
    ...
    def "should complete if AtomicReference<BluetoothGatt> contains null and then release the radio"() {
        ...
        then:
        mockGattCallback.getBluetoothGatt() >> Observable.just(null)
    ...
    private prepareObjectUnderTest() {
        def rxBleDevice = Mock(RxBleDevice)
        rxBleDevice.getMacAddress() >> mockMacAddress
        objectUnderTest = new RxBleRadioOperationDisconnect(mockGattCallback, rxBleDevice, mockBluetoothManager,
                ImmediateScheduler.INSTANCE, new MockOperationTimeoutConfiguration(Schedulers.computation()))
